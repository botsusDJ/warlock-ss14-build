...
///      превращается в груз.
///   2. Усиление удара. Базовый множитель рамы достаётся и кулакам, и оружию в руках:
///      приводы усиливают руку, а не кулак. Ползунок в ОС только доводит один канал
///      поверх базы, отнять у другого он не может.
...
public sealed partial class WarlockExosuitSystem : EntitySystem
{
    ...
    public void Apply(Entity<WarlockExosuitComponent> ent, EntityUid wearer)
    {
        ...
        {
            // Базовая мощность рамы идёт в оба канала целиком. Раньше она между ними
            // делилась, и рама с прибавкой в треть давала предмету считанные проценты —
            // боец в броне бил ломом почти как без неё. Приводы усиливают руку, а не
            // кулак: чем рука бьёт, раме безразлично.
            var power = 1f + (ent.Comp.StrengthBonus - 1f) * CellOutput(ent);

            // Ползунок распределения остался, но теперь он только доводит один канал
            // поверх базы, а не отнимает у другого. Ниже базы не опускается ничто.
            //
            //   боевая рама, поровну          -> оба x2.65
            //   боевая рама, перекос в кулаки -> кулак x2.9, предмет x2.4
            var swing = ent.Comp.ShareSwing;
            var fist = ent.Comp.FistShare;
            var tool = 1f - fist;

            w.FistBonus = power + swing * fist;
            w.ToolBonus = power + swing * tool;
