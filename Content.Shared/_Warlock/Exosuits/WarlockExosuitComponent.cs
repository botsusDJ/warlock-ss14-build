...
public sealed partial class WarlockExosuitComponent : Component
{
    ...
    /// <summary>
    /// Во сколько раз рама умножает урон в ближнем бою. Двойка — вдвое.
    ///
    /// Это множитель, а не прибавка к одному каналу: он целиком достаётся и кулакам,
    /// и тому, что боец держит в руках. Ползунок распределения только доводит один
    /// канал поверх этого числа, отнять у другого он не может.
    /// </summary>
    [DataField, AutoNetworkedField]
    public float StrengthBonus = 2.4f;
    ...
    #region Перекос привода

    /// <summary>
    /// Куда уходит перекос: единица — весь в кулаки, ноль — весь в кисти.
    /// Делится только надбавка <see cref="ShareSwing"/>; базовый множитель рамы
    /// оба канала получают целиком в любом положении ползунка.
    /// </summary>
    [DataField, AutoNetworkedField]
    public float FistShare = 0.5f;

    /// <summary>
    /// Сколько множителя добавляет полный перекос ползунка в свою сторону.
    /// При 0.5 выкрученный до упора канал получает +0.5 к множителю поверх базы,
    /// а противоположный остаётся ровно на базе.
    /// </summary>
    [DataField]
    public float ShareSwing = 0.5f;
...
/// <summary>
/// _Warlock — игрок нажал пуск или останов приводов в ОС.
///
/// Отдельным сообщением, а не флагом в настройках: настройки применяются кнопкой
/// целиком, а приводы должны запускаться и глохнуть мгновенно.
/// </summary>
[Serializable, NetSerializable]
public sealed class WarlockExoOsToggleMessage : BoundUserInterfaceMessage;
